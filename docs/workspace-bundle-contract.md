...
- 8 张当前成品态 Entry / Document 标签及精确 Fragment 区间表；
- 2 张 Entry 联系投影与当前控制表；
- 14 张处理任务、提案信封、AI 拆分/标签/联系提案、自动路由执行、所有者工作队列及确定性动作表；
- 所有被 Evidence 引用的原始/规范化 Blob 字节；
- 当前工作区的快捷标签、自动标签筛选、标签别名、Entry Profile、AutomationPolicy、联系/探索策略和信源订阅偏好。

共计 61 张数据库表。运行配置、数据库 URL、密码、API key、访问令牌、外部工作区路径和其他秘密不属于个人数据包，继续由外部配置/秘密机制提供。
...
当前默认完整包上限为 256 MiB、1,000,000 个 canonical JSON value。Blob 字节在个人数据分区中恰好保存一份 base64 表示，同时信封继续保留独立 Blob identity 清单用于引用闭包核验。每个字节块必须同时满足声明长度和 SHA-256；导入时不能用路径或文件名替代内容身份。
...
| `struinfo.m1d-entry`       |       4 | `struinfo.m1d-entry.v4`       | Entry 当前结构事实、Fragment 输入/可选精确区间、三维标签、lineage 与 Document 标签，8 表 |
| `struinfo.m1d-association` |       4 | `struinfo.m1d-association.v4` | 可重建联系投影、当前人工控制与正式图谱关系语义/说明/核验元数据，2 表                     |
| `struinfo.m1e-processing`  |       7 | `struinfo.m1e-processing.v7`  | 处理任务、提案载荷、typed 自动路由、所有者队列与确定性动作，14 表                        |
...
Entry v5、Association v4 和 Processing v7 反映当前成品态。Entry v3 为人工物化增加可选半开 Unicode 标量区间；没有区间行的输入仍表示完整 Fragment。Entry v4 在相同八张表内为每个 Entry 增加 `is_current_structure`；Entry v5 增加第九张 `information_document_working_copy` 表。Association v3 在当前 override 上增加关系来源、名称和方向；Association v4 再增加闭合语义、500 code-point 内维护说明和未核验/已核对来源/需复核状态，并以保守默认值读取 v1–v3。Entry 的 `current_revision` / `current_revision_id` 只作为乐观并发版本令牌，不再指向一张普通编辑历史表。Processing v7 保存 typed 任务、提案、路由和确定性动作状态，不保存 Entry 正文、API key、提示全文、原始模型响应或任意 JSON。不可变 Snapshot、Fragment 以及正式 Knowledge 修订历史仍由 M1C domain 分区原样保留。
...
1. 在 PostgreSQL `REPEATABLE READ READ ONLY` 快照中读取当前 61 张工作区表；
...
- 带 `struinfo.m1d-entry.v1`：只保留旧指针选中的当前 Entry/Document 标签值，补空的 Fragment 区间表并转换为 v4；`struinfo.m1d-entry.v2` 原样保留七张当前态表并补空区间表；`struinfo.m1d-entry.v3` 保留精确区间；三种旧版本都把既有 Entry 补为 `is_current_structure=true`；
- 带 `struinfo.m1d-association.v1` 或 `.v2`：保留当前人工控制并转换为 v3；缺失的图谱关系元数据为 `null`；
- 缺少 `struinfo.m1e-processing`：处理任务、提案和自动路由状态按空处理；Processing v1 保留任务、提案信封和 Fragment 输入，v2 增加 AI 标签提案，v3 增加 selected-pair 联系提案，v4 增加 AI split 提案，v5 增加自动路由运行与 claim，v6 增加所有者工作队列；读取 v1–v6 时缺失的后续表按空处理并转换为 v7；
